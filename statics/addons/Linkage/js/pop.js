function open_linkage(id,name,container,linkageid) {
	...
	{
		content += '<a style="padding:3px 5px;" href="javascript:;" onclick="get_child(\''+container[i][0]+'\',\''+container[i][1]+'\',\''+container[i][2]+'\')">'+container[i][1]+'</a>';
	}
	content += '</div></div>';
	art.dialog({title:name,id:'edit_'+id,content:content,width:'522px',height:'auto',style:'none_icon ui_content_m'});
}
var level = 0;
function get_child(nodeid,nodename,parentid) {
	var content = container = '';
	var url = "index.php?g=Linkage&m=Index&a=ajax_getlist&parentid="+nodeid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			level = 1;
			$.each(data, function(i,data){ 
				container = data.split(',');
				content += '<a href="javascript:;" style="padding:3px 5px;" onclick="get_child(\''+container[0]+'\',\''+container[1]+'\',\''+container[2]+'\')">'+container[1]+'</a>';	
			})
...
function get_parent(obj,parentid) {
	...
	var content = container = '';
	var url = "index.php?g=Linkage&m=Index&a=ajax_getlist&linkageid="+linkageid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$.each(data, function(i,data){ 
				container = data.split(',');
...
function get_path(nodeid,returnid) {
	var show_path = '';
	var url = "index.php?g=Linkage&m=Index&a=ajax_getpath&parentid="+nodeid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$.each(data, function(i,data){ 
...
function set_val(nodename,nodeid) {
	...
	var url = "index.php?g=Linkage&m=Index&a=ajax_gettopparent&linkageid="+nodeid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$("#city").val(data);
		}
