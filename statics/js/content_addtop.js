//新窗口打开
function omnipotent(id, linkurl, title, id, w, h, fullscreen) {
	var offset = 0;
	if(!id) id = '';
    ...
    layer.open({
        type: 2,
        title: '<img src="'+GV.DIMAUB+'statics/images/copy.png"><font class="f15 b pl5">' + title + '</font>',
        maxmin: true,
        id:id,
        shade: 0.6,
...
function show_relation(modelid, id) {
    $.getJSON(GV.DIMAUB+"index.php?a=public_getjson_ids&m=Contents&g=Contents&modelid=" + modelid + "&id=" + id, function (json) {
        var newrelation_ids = '';
        if (json.data == null) {
            isalert('没有添加相关内容！');
            return false;
...
//显示相关内容
function show_relation_url(url, modelid, id) {
    $.getJSON(url+"&modelid=" + modelid + "&id=" + id, function (json) {
        ...
        $.each(json.data, function (i, n) {
            newrelation_ids += "<li id='" + n.sid + "'>·<span>" + n.name + "</span><a href='javascript:;' class='close' onclick=\"remove_relation('" + n.sid + "'," + n.id + ")\"></a></li>";
        });
