                ...
                form.ajaxSubmit({
                    ...
                    success: function (data, statusText, xhr, $form) {
                        ...
                        if (data.state === 'success') {
                            ...
                            $('.J_ajax_submit_btn').removeProp('disabled').removeClass('disabled').removeAttr('disabled');
                        }
                    }
                });
            });
...
function openwinx(url,name,w,h) {
    if(!w) w= ($(window).width() - 100) ;
    if(!h) h= ($(window).height() );
    if(!name) name = '基本信息';
    //window.open(url,name,"top=100,left=400,width=" + w + ",height=" + h + ",toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=no");
    layer.open({
        type: 2,
        title: '<img src="'+GV.DIMAUB+'statics/images/copy.png"><font class="f15 b pl5">' + name + '</font>',
        maxmin: true,
        shadeClose: true,
        shade: 0.5,
        offset: ['0px', '100px'],
...
function previewInput(){
	var preicon = $('#icon').val();
	var iconHtml = '<i class="fa '+preicon+'"></i>';
	$('#preicon').html(iconHtml);
	
	var suficon = $('#icon2').val();
	var iconHtml2 = '<i class="fa '+suficon+'"></i>';
	$('#suficon').html(iconHtml2);
	
	var pretext = ' ' + $('#ftext').val();
	$('#pretext').html(pretext);
	
	var suftext = ' ' + $('#ftext2').val();
	$('#suftext').html(suftext);
}