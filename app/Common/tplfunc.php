...
function tpl_wappage_editor($editorparams = '', $editormodules = array()) {
    ...
    // 编译并加载模板文件
	import('Dir');
	$Dir = new Dir(APP_PATH . 'Modules/Site/Tpl/JSTemplate/wapeditor');
	$filetree = $Dir->toArray();
	if (!empty($filetree)) {
		foreach ($filetree as $file) {
			if (strpos($file['filename'], 'widget-') !== FALSE) {
				$filename = substr($file['filename'], 0, strpos($file['filename'], '.')-1);				
				$param['iseditor'] = false;
				$param['filename'] = $filename . '-display';
				$tpl->fetch($file['pathname'], $param);
				$param['iseditor'] = true;
				$param['filename'] = $filename . '-editor';
				$tpl->fetch($file['pathname'], $param);
			}
		}
	}
...
function tpl_ueditor($id, $value = '', $options = array()) {
	...
	if (!defined('TPL_INIT_UEDITOR')) {
		$s .= '<script type="text/javascript" src="/statics/resource/components/ueditor/ueditor.config.js"></script><script type="text/javascript" src="/statics/resource/components/ueditor/ueditor.all.min.js"></script><script type="text/javascript" src="/statics/resource/components/ueditor/lang/zh-cn/zh-cn.js"></script>';
	}
