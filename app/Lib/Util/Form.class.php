...
class Form {
	 ...
	 * @param $html 需要显示的Html效果
     */
    public static function date($name, $value = '', $isdatetime = 0, $loadjs = 0, $showweek = 'true', $timesystem = 1, $html = '') {
        ...
        $input = '<input type="text" name="' . $name . '" id="' . $id . '" value="' . $value . '" size="' . $size . '" class="form-control input length_3 J_datetime " aria-describedby="span_'.$id.'">';
		if($html) return sprintf($html, $input);
		return $input;
    }

    /**
     * 分类选择
     * @param string $file 分类缓存文件名
     * @param intval/array $catid 别选中的ID，多选是可以是数组
     * @param string $str 属性
     * @param string $default_option 默认选项
     * @param intval $modelid 按所属模型筛选
     * @param intval $type 分类类型
     * @param intval $onlysub 只可选择子分类
    ...
    public static function checkbox($array = array(), $id = '', $str = '', $defaultvalue = '', $width = 0, $field = '') {
        ...
        foreach ($array as $key => $value) {
            ...
            if ($width) $string .= '<label class="ib">';
