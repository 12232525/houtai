...
class ModelModel extends CommonModel {
    ...
    const commonModelSql = 'Sql/appcms_common.sql'; //一般模型
    const commentModelSql = 'Sql/appcms_comment.sql'; //评论回复类型
    const flowModelSql = 'Sql/appcms_flow.sql'; //审批工作流类型
    
    const mainTableSql = 'Sql/appcms_zhubiao.sql'; //模型主表SQL模板文件
    const sideTablesSql = 'Sql/appcms_zhubiao_data.sql'; //模型副表SQL模板文件
    const modelTablesInsert = 'Sql/appcms_insert.sql'; //可用默认模型字段
    const membershipModelSql = 'Sql/appcms_member.sql'; //会员模型
    ...
    public function checkTablesql() {
        //检查主表结构sql文件是否存在
        if (!is_file($this->libPath . self::commonModelSql)) {
            return false;
        }
		if (!is_file($this->libPath . self::commentModelSql)) {
            return false;
        }
		if (!is_file($this->libPath . self::flowModelSql)) {
            return false;
        }
    ...
    public function AddModelMember($tableName, $modelId) {
        ...
        //表前缀，表名，模型id替换
        $sqlSplit = str_replace(array('@app@', '@zhubiao@', '@modelid@'), array($dbPrefix, $tableName, $modelId), $membershipModelSql);
    ...
    public function addModel($data) {
        ...
        if ($data) {
            ...
            if ($modelid) {
                //创建数据表
                if ($this->createModel($data['tablename'], $modelid, $data['type'])) {
                    return $modelid;
                } else {
                    //表创建失败
     ...
     * @return boolean
     */
    public function copyModel($data) {
        if (empty($data)) {
            return false;
        }
        //数据验证
        $table_name = $data['tablename'];
        $copy_table_name = $data['copy_table_name'];
        $data = $this->create($data, 1);
        if ($data) {
            //添加模型记录
            $modelid = $this->add($data);
            if ($modelid) {
                //创建数据表
                if ($this->executeCopyModel($copy_table_name, $modelid, $table_name)) {
                    return $modelid;
                } else {
                    //表创建失败
                    $this->where(array("modelid" => $modelid))->delete();
     ...
     * @return boolean
     */
    protected function executeCopyModel($copyTableName, $modelId, $table_name)
    {
        if (empty($copyTableName) || $modelId < 1) {
            return false;
        }

        $copyTableName = substr($copyTableName, strlen(C("DB_PREFIX")));
        $res = $this->where(array('tablename'=>$copyTableName))->find();
        if(!$res) return false;

        $copyModelId = $res['modelid'];
        $FieldModel = D('Model_field');
        $fieldList = $FieldModel->getModelField($copyModelId);

        if(!$fieldList) return false;

        foreach ($fieldList as $key => $val){
                $fieldList[$key]['modelid'] = $modelId;
                unset($fieldList[$key]['fieldid']);
        }

        $this->startTrans();

        $sql = 'CREATE TABLE ' . C("DB_PREFIX") . $table_name . ' LIKE ' . C("DB_PREFIX") . $copyTableName;
        $createFlag = $this->sql_execute($sql);

        $copyFieldFlag = $FieldModel->addAll($fieldList);

        if($createFlag && $copyFieldFlag) {
            $this->commit();
            return true;
        }
        $this->rollback();
     ...
     * @return boolean
     */
    protected function createModel($tableName, $modelId, $modelType = 3) {
        ...
        if($modelType == 0){  //文章类型时
       	 	//读取模型主表SQL模板
        	$mainTableSql = file_get_contents($this->libPath . self::mainTableSql);
        	//副表
        	...
        	//表前缀，表名，模型id替换
        	$sqlSplit = str_replace(array('@app@', '@zhubiao@', '@modelid@'), array($dbPrefix, $tableName, $modelId), $mainTableSql . "\n" . $sideTablesSql . "\n" . $modelTablesInsert);
 		
		}elseif($modelType == 3){  //一般模型类型时
			
 			//读取模型主表SQL模板
        	$commonTableSql = file_get_contents($this->libPath . self::commonModelSql);
			//表前缀，表名，模型id替换
        	$sqlSplit = str_replace(array('@app@', '@zhubiao@', '@modelid@'), array($dbPrefix, $tableName, $modelId), $commonTableSql);
 		}elseif($modelType == 4){  //评论回复类型
			
 			//读取模型主表SQL模板
        	$commentTableSql = file_get_contents($this->libPath . self::commentModelSql);
			//表前缀，表名，模型id替换
        	$sqlSplit = str_replace(array('@app@', '@zhubiao@', '@modelid@'), array($dbPrefix, $tableName, $modelId), $commentTableSql);
 		}elseif($modelType == 5){  //审批工作流类型
			
 			//读取模型主表SQL模板
        	$flowTableSql = file_get_contents($this->libPath . self::flowModelSql);
			//表前缀，表名，模型id替换
        	$sqlSplit = str_replace(array('@app@', '@zhubiao@', '@modelid@'), array($dbPrefix, $tableName, $modelId), $flowTableSql);
 		}
    ...
    public function sql_split($sql) {
        if (mysqli_get_server_info() > '4.1' && C('DB_CHARSET')) {
            $sql = preg_replace("/TYPE=(InnoDB|MyISAM|MEMORY)( DEFAULT CHARSET=[^; ]+)?/", "ENGINE=\\1 DEFAULT CHARSET=" . C('DB_CHARSET'), $sql);
        }
        if (C("DB_PREFIX") != "appcms_") {
            $sql = str_replace("appcms_", C("DB_PREFIX"), $sql);
        }
        $sql = str_replace("\r", "\n", $sql);
        $ret = array();
        $num = 0;
