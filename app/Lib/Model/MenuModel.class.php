...
class MenuModel extends CommonModel {
     ...
     */
    public function admin_menu($parentid, $with_self = false, &$data) {
        //父节点ID
        $parentid = (int) $parentid;
        //if(!$result)  $result = $this->where(array('parentid' => $parentid, 'status' => 1))->order(array("listorder" => "ASC"))->select();
		$result = array();
		foreach($data as &$val){
    ...
    public function menu_json() {
        ...
        $changyong = array(
            "changyong" => array(
                "icon" => 'fa-tachometer',
                "icontype" => 3,
                "logo" => '/statics/admin/theme/default/images/app/app-home.png',
                "font" => 'f24',
                "id" => "changyong",
                "name" => "仪表盘",
                "parent" => "",
    	...
    	$result = $this->where(array('status' => 1))->order(array("listorder" => "ASC"))->select();
		
		//第1步业务权限：先判断业务模型权限
		import('RbacModule', LIB_PATH . 'Module');
		$rbacModule = new RbacModule();
		$moduleMenuList = $rbacModule->getModuleMenu($result);
		
		//第2步操作权限：在判断相应的操作是不是拥有权限
        $operateMenuList = $this->get_tree(0, '', 1, $moduleMenuList);
		
        return array_merge($changyong, $operateMenuList);
    }

    //取得树形结构的菜单
    public function get_tree($myid, $parent = "", $Level = 1, &$result) {
        $data = $this->admin_menu($myid, false, $result);
        ...
        if (is_array($data)) {
            foreach ($data as $a) {
                ...
                $ret[$id . $name] = $array;
                $child = $this->get_tree($a['id'], $id, $Level, $result);
                //由于后台管理界面只支持三层，超出的不层级的不显示
                if ($child && $Level <= 3) {
                	$ret[$id . $name]['hasChild'] = $child ? 1 : 0;
                    $ret[$id . $name]['items'] = $child;
                }else{
                	//如果是第二级菜单，并且下面没有子菜单时，将会进行移除二级菜单项
					if($Level == 3) unset($ret[$id . $name]);
                }
            }
        }
        return $ret;
