...
class OmnipotentField {
	...
	public function getInfoList(&$data, $fieldList) {
		...
		foreach($fieldList as $column => $va){
			...
			if(isset($arr['table_name']) && $arr['table_name']) {
				$tableList[$column] = substr($arr['table_name'], strlen(C("DB_PREFIX"))-1);
			}	
		}
		...
		foreach($tableList as $colu => $table){
			...
			$res = M($table)->where($where)->field('id,name')->select();
