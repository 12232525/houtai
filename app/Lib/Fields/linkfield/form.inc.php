...
function linkfield($field, $value, $fieldinfo) {
    ...
    if ($setting['link_type']) {
		...
		$sel_id = $setting['set_id'];

        $sql = "SELECT " . $sel_tit . " FROM `" . $setting['table_name'] . "` ORDER BY $sel_id desc ";
        $dataArr = $get_db->query($sql);

        //$dataArr = $get_db->field($sel_tit)->order( $sel_id . " desc")->select();

        $value = str_replace('&amp;', '&', $value);
        $data = '<input type="text" name="info[' . $fieldinfo['field'] . ']" id="' . $fieldinfo['field'] . '" value="'.$value.'" class="input"  style="display:none;"><select id="set_' . $fieldinfo['field'] . '" onchange="$(\'#' . $fieldinfo['field'] . '\').val(this.value);">';
		
		if($setting['default_opt']){
			$selectStr = '';
			if($value == 0) $selectStr = 'selected';
			$data .= "<option value='0' " . $selectStr . ">" . $setting['default_opt'] . "</option>\n";
		}else{
        ...
        $data = <<<EOT
            ...
            $(document).ready(function(){
				if($("#{$fieldinfo['field']}").val().length > 0){
					...
					$.getJSON('{$domain}api.php?m=Ajaxlinkfield&a=public_index&act=check_search&key={$key}&callback=?', {value: value,table_name: tablename,set_title: set_title,set_id: set_id,set_type: set_type,select_title:select_title,random:Math.random()}, function(data2){
						if (data2 != null) {
				...
				$('#cat_search{$field}').keyup(function(){
					...
					if (value.length > 0){
						$.getJSON('{$domain}api.php?m=Ajaxlinkfield&a=public_index&act=search_ajax&key={$key}&callback=?', {value: value,table_name: tablename,select_title: select_title,like_title: like_title,set_where:set_where,set_title: set_title,set_id: set_id,limit: 20,random:Math.random()}, function(data){
							if (data != null) {
								var str = '';
