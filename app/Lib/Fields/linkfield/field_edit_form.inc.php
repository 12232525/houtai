...
<?php } ?>
    ...
    <tr> 
        <td>默认显示内容</td>
        <td>
        	<input type="text" name="setting[default_opt]" value="<?php echo $setting['default_opt']; ?>"  size="40" class="input"/> 
        </td>
    </tr>
    <tr> 
        <td>展现形式</td>
        <td>
        	<input type="radio" name="setting[display_css]" value="select" <?php if ($setting['display_css'] == 'select') echo 'checked'; ?>/>一般下拉框
        	<input type="radio" name="setting[display_css]" value="tree" <?php if ($setting['display_css'] == 'tree') echo 'checked'; ?>/>下拉框树形结构
        </td>
    </tr>
</table>