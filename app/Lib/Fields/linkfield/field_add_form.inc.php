...
<table cellpadding="2" cellspacing="1" onclick="javascript:$('#minlength').val(0);$('#maxlength').val(255);">
    ...
    <tr> 
        <td>存入数据方式</td>
        <td>
        	<input type="radio" name="setting[insert_type]" value="id" checked="checked"/>主键<input type="radio" name="setting[insert_type]" value="title"/>赋值字段<input name="setting[insert_type]" type="radio" value="title_id"/>
            赋值字段+主键
        </td>
    </tr>
    <tr> 
        <td>默认显示内容</td>
        <td>
        	<input type="text" name="setting[default_opt]" value=""  size="40" class="input"/> 
        </td>
    </tr>
    <tr> 
        <td>展现形式</td>
        <td>
        	<input type="radio" name="setting[display_css]" value="select" checked="checked"/>一般下拉框
        	<input type="radio" name="setting[display_css]" value="tree"/>下拉框树形结构
        </td>
    </tr>
</table>