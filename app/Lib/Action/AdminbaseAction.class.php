...
class AdminbaseAction extends AppframeAction {
	...
	public function echoAjaxReturn($flag, $data = '', $message = 'success'){
		$res = array();
	...
	/**
	 * 根据Model获取数据
	 * 如果$needPage = true：需要分页，则返回array('list'=> , 'page'=> );
	 * 如果$needPage = flase：不需要分页，直接返回list结果集
	 */
	public function getModelInfoList($table_name, $where = '', $needPage = false){
		if(!$table_name) return array();
		
		import('RenderModelModule', LIB_PATH . 'Module');
		$renderModelModule = new RenderModelModule();
		$result = $renderModelModule->getModelInfoListForId2Value($table_name, $where, $needPage);
		
		if(!$needPage && $result) return $result['list'];
		return $result;
	}
