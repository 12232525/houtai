 ...
 */
class TagLibApp extends TagLib {

    // 数据库where表达式
    protected $comparisonAppcms = array(
        '{notlike}' => 'NOT LIKE',
        '{notin}' => 'NOT IN',
        '{like}' => 'LIKE',
        '{eq}' => '=',
     ...
     * 参数说明：
     *          @catid		分类id，可以传入数字，也可以传递变量 $catid
     *          @space		分隔符，支持html代码
     *          @blank		是否新窗口打开
     *          @cache                              缓存时间
     ...
     * 		@num		每次返回数据量
     * 		@catid		分类id（必填），列表页，内容页可以使用 $catid 获取当前分类。
     * 	公用参数：
     * 		@cache		数据缓存时间，单位秒
     * 		@pagefun                      分页函数，默认page
     * 		@pagetp		分页模板，必须是变量传递
     * 		@return		返回值变量名称，默认data
     * 	#当action为lists时，调用分类列表标签
     * 	#用法示例：<content action="lists" catid="$catid"  order="id DESC" num="4" page="$page"> .. HTML ..</content>
     * 	独有参数：
     * 		@order		排序，例如“id DESC”
     ...
     * 标签：<get></get>
     * 作用：特殊标签，SQL查询标签
     * 用法示例：<get sql="SELECT * FROM appcms_article  WHERE status=99 ORDER BY inputtime DESC" page="$page" num="5"> .. HTML ..</get>
     * 参数说明：
    ...
    public function _get($attr, $content) {
        ...
        $tag['sql'] = $sql = str_replace(array("think_", "appcms_"), C("DB_PREFIX"), strtolower($tag['sql']));
        //数据源
        ...
        if ($page) {
            ...
            if ($_sql = preg_replace('/select([^from].*)from/i', "SELECT COUNT(*) as count FROM ", $tag['sql'])) {
                ...
                if (substr(trim($sql), 0, 1) == '$') {
                    $parseStr .= ' $sql = str_replace(array("think_", "appcms_"), C("DB_PREFIX"),' . $sql . ');';
            ...
            //判断是否变量传递
            if (substr(trim($sql), 0, 1) == '$') {
                $parseStr .= ' $_sql = str_replace(array("think_", "appcms_"), C("DB_PREFIX"),' . $sql . ');';
            } else {
                $parseStr .= ' $_sql = "' . str_replace('"', '\"', $sql) . '";';
            }
            $parseStr .= ' $cache = ' . $cache . ';';
    ...
    protected function parseSqlCondition($condition) {
        $condition = str_ireplace(array_keys($this->comparisonAppcms), array_values($this->comparisonAppcms), $condition);
        return $condition;
    }
