...
class ContentTagLib {
     ...
     * 
     * moreinfo参数属性，本参数表示在返回数据的时候，会把副表中的数据也一起返回。一个内容模型分为2个表，一个主表一个副表，主表中一般是保存了标题、所属分类等等短小的数据（方便用于索引），而副表则保存了大字段的数据，如内容等数据。在模型管理中新建字段的时候，是允许你选择存入到主表还是副表的（我们推荐的是，把不重要的信息放到副表中）。
     * @param $data
     */
    public function lists($data) {
        ...
        //把数据组合成以id为下标的数组集合
        if ($dataList) {
            $return = $url_arr = array();
            foreach ($dataList as $r) {
            	$url_arr = $this->url->show($r);
            	$r['url'] = $url_arr['url'];
                $return[$r['id']] = $r;
            }
        } else {
    ...
    public function listsNew($data) {
        ...
        //log
        if (APP_DEBUG) {
            $msg = "ContentTagLib标签->lists：参数：newList=1 , modelid=$modelid ,order=" . $data['order'] . " ,
            SQL:" . $getLastSql;
            Log::write($msg);
        }
        return $return;
    ...
    public function relation($data) {
        ...
        if ($data['keywords'] && $limit > $number) {//根据关键字的相关内容
     ...
     * 分类列表（category）
     * 参数名	 是否必须	 默认值	 说明
     * catid	 否	 0	 调用该分类下的所有分类 ，默认0，调用一级分类
     * order	 否	 null	 排序方式、一般按照listorder ASC排序，即分类的添加顺序
     * @param $data
     */
