 ...
 * Contact email:it100@qq.com
 */
class BakdataModel extends Model {
    ...
    public function getSqlFilesList() {
        ...
        while ($file = readdir($handle)) {
            if (preg_match('#\.sql$#i', $file)) {
                ...
                $bk['time'] = date("Y-m-d H:i:s", fileatime(DatabaseBackDir . "/$file"));
     ...
     * @return array
      +----------------------------------------------------------
     */
    public function zip($files, $filename, $outDir = SITE_PATH, $path = DatabaseBackDir) {
        $zip = new ZipArchive;
        makeDir($outDir);
