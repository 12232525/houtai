...
class CronModel extends CommonModel {
    ...
    public function CronAdd($data) {
        ...
        switch ($type) {
            case 1:
                if (!$data['catid']) {
                    $this->error = "请选择需要刷新的分类！";
                    return false;
                }
                $catid = implode(",", $data['catid']);
                unset($data['catid']);
                $data['data'] = $catid;
                $data['cron_file'] = 'AppCMSRefresh_category';
                break;
            case 2:
                if (!$data['tempid']) {
                ...
                unset($data['tempid']);
                $data['data'] = $tempid;
                $data['cron_file'] = 'AppCMSRefresh_custompage';
                break;
            case 3:
                $data['cron_file'] = 'AppCMSRefresh_index';
                $data['data'] = "";
                break;
            default :
                $data['data'] = "";
    ...
    public function _getCronFileList() {
        ...
        foreach ((array) $fileList AS $k => $file) {
            if (strpos($file['filename'], "AppCMS") !== 0) {
                unset($fileList[$k]);
            } else {
