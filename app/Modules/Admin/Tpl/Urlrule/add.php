...
<div class="wrap J_check_wrap">
  ...
  <div class="prompt_text">
    <p>实现伪静态地址，规则需要自己写，同时也要服务器支持！<br/>
      例如实现伪静态这样的：http://www.it100.net/tag-标签.html ，URL规则是：tag-{$tag}.html|/tag-{$tag}-{$page}.html，服务器上设置，如果是.htaccess则规则是：RewriteRule ^(tag|Tag)-(.*?)\.html$ index.php?g=Tags&m=Index&a=index&tag=$2</p>
  </div>
  ...
  <form action="{:U("Urlrule/add")}" method="post" name="myform" class="J_ajaxForm">
    <div class="table_full">
      <table width="100%" cellpadding="2" cellspacing="1" class="table_form">
        ...
        <tr>
          <th>URL规则 :</th>
          <td><input type="text" class="input" name="urlrule" id="urlrule" value="{$data.urlrule}" size="70"><br/>如果以“=”开头，表示以自定义处理函数返回路径。函数存放于app\Common\urlrule.php文件中。</td>
        </tr>
        <tr>
          <th>可用变量 :</th>
          <td>父分类路径：
