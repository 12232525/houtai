 ...
 */
class RbacModuleAction extends AdminbaseAction {
	...
	public function ajaxNavShow(){
		...
		$html = $this->fetch('RbacModule:Nav:detail_comment_flow');	
		echo $html;
		exit;
	}
     ...
     * 设置模型权限 
     */
    public function setting_model_priv() {
        if (IS_POST) {
            $roleid = $this->_post("roleid");
            $priv = array();
            foreach ($_POST['priv'] as $k => $v) {
                foreach ($v as $e => $q) {
                    $priv[] = array("roleid" => $roleid, "modelid" => $k, "action" => $q, "is_admin" => 1);
                }
            }
            C('TOKEN_ON', false);
            //循环验证每天数据是否都合法
            foreach ($priv as $r) {
                $data = D("Model_priv")->create($r);
                if (!$data) {
                    $this->error(D("Model_priv")->getError());
                } else {
                    $addpriv[] = $data;
                }
            }
            C('TOKEN_ON', true);
            //设置权限前，先删除原来旧的权限
            M("Model_priv")->where(array("roleid" => $roleid))->delete();
            //添加新的权限数据，使用D方法有操作记录产生
            D("Model_priv")->addAll($addpriv);
            $this->success("权限赋予成功！");
        } else {
            $roleid = $this->_get("roleid");
            $categorys = D("Model")->where('type = 3 AND disabled = 0')->select();
            $category_priv = M("Model_priv")->where(array("roleid" => $roleid))->select();
            $priv = array();
            foreach ($category_priv as $k => $v) {
                $priv[$v['modelid']][$v['action']] = true;
            }
            ...
            foreach ($categorys as $k => $v) {
                if ($v['type'] == 1 || $v['child']) {
                    ...
                    $v['disabled'] = '';
                    $v['add_check'] = isset($priv[$v['modelid']]['add']) ? 'checked' : '';
                    $v['delete_check'] = isset($priv[$v['modelid']]['delete']) ? 'checked' : '';
                    $v['listorder_check'] = isset($priv[$v['modelid']]['listorder']) ? 'checked' : '';
                    $v['push_check'] = isset($priv[$v['modelid']]['push']) ? 'checked' : '';
                    $v['move_check'] = isset($priv[$v['modelid']]['remove']) ? 'checked' : '';
                    $v['edit_check'] = isset($priv[$v['modelid']]['edit']) ? 'checked' : '';
                }
                $v['init_check'] = isset($priv[$v['modelid']]['init']) ? 'checked' : '';
                $categorys[$k] = $v;
            }
			...
			foreach ($categorys as $k => $v) {
				$modelid = $v['modelid'];
				$init_check = $v['init_check'];
				$disabled = $v['disabled']; 
				$add_check = $v['add_check']; 
				$edit_check = $v['edit_check'];
				$delete_check = $v['delete_check'];
				$listorder_check = $v['listorder_check'];
				$push_check = $v['push_check'];
				$move_check = $v['move_check'];
				$str .= "<tr>
							<td align='center'><input type='checkbox'  value='1' onclick='select_all($modelid, this)' ></td>
							<td align='center'>".$v['modelid']."</td>
							<td>".$v['name']."</td>
							<td align='center'><input type='checkbox' name='priv[$modelid][]' $init_check  value='init' ></td>
							<td align='center'><input type='checkbox' name='priv[$modelid][]' $disabled $add_check value='add' ></td>
							<td align='center'><input type='checkbox' name='priv[$modelid][]' $disabled $edit_check value='edit' ></td>
							<td align='center'><input type='checkbox' name='priv[$modelid][]' $disabled $delete_check  value='delete' ></td>
							<td align='center'><input type='checkbox' name='priv[$modelid][]' $disabled $listorder_check value='listorder' ></td>
							<td align='center'><input type='checkbox' name='priv[$modelid][]' $disabled $push_check value='push' ></td>
							<td align='center'><input type='checkbox' name='priv[$modelid][]' $disabled $move_check value='remove' ></td>
            		    </tr>";
			}	
            $this->assign("categorys", $str);
            
            $this->assign("roleid", $roleid);
            $this->display("modelrbac");
        }
    }
