...
class MenuAction extends AdminbaseAction {
    ...
    public function index() {
        ...
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        foreach ($result as $r) {
            $r['str_manage'] = '<div class="btn-toolbar"><div class="btn-group"><a class="btn pl5 pr5" href="javascript:void(0)" onclick="omnipotent(\'icadd\', \'' . U("Menu/add", array("parentid" => $r['id'], "menuid" => $_GET['menuid'])) . '\', \'添加'.$r['name'].'子分类\');"><i class="fa fa-plus"></i></a><a class="btn pl5 pr5" href="javascript:void(0)" onclick="omnipotent(\'icedit\',\'' . U("Menu/edit", array("id" => $r['id'], "menuid" => $_GET['menuid'])) . '\', \'编辑'.$r['name'].'分类\');"><i class="fa fa-pencil-square-o"></i></a><a class="J_ajax_del btn pl5 pr5" href="' . U("Menu/delete", array("id" => $r['id'], "menuid" => $this->_get("menuid"))) . '"><i class="fa fa-trash-o"></i></a> </div></div>';
            $r['status'] = $r['status'] ? '<font color="red"><i class="fa fa-check-circle"></i></font>' : '<font color="red"><i class="fa fa-times-circle"></i></font>';

            $r['logo'] = $r['logo'] ? "<img src='".$r['logo']."' width='20' height='20'/>" : '';
            $r['icon'] = $r['icon'] ? "<i class='fa ".$r['icon']."'></i>" : '';

            $r['pfont'] =  $r['parentid'] == 0 ? '<a href="javascript:;" data-id="'.$r['id'].'" data-pid="'.$r['parent_id'].'" class="btn btn-mini btn-success btn-node-sub"><i class="fa fa-sitemap"></i></a>' : '<a href="javascript:;" data-id="'.$r['id'].'" data-pid="'.$r['parent_id'].'" class="btn btn-mini btn-default btn-node-sub"><i class="fa fa-sitemap"></i></a>';

            $r['type'] = $r['type'] == 1 ? '权限+菜单' : '菜单';
            $r['redirect'] = $r['redirect'] ? '<font color="red"><i class="fa fa-check-circle"></i></font>' : '<font color="red"><i class="fa fa-times-circle"></i></font>';

            $array[] = $r;
        }

        $tree->init($array);
        $str = "<tr data-index='\$id' data-parent='\$parentid'>
    ...
    public function add() {
        if (IS_POST) {
        	...
        	$authkey = upload_key("1,gif|jpg|jpeg|png,1,,,0");
