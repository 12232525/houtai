...
class RbacAction extends AdminbaseAction {
    ...
    public function setting_model_priv() {
        if (IS_POST) {
            ...
            $roleid = $this->_get("roleid");
            $categorys = D("Model")->where('type in (3,4,5) AND disabled = 0')->order('sort asc')->select();
            ...
            foreach ($categorys as $k => $v) {
                if ($v['type'] == 1 || $v['child']) {
                    ...
                    $v['flow_check'] = isset($priv[$v['modelid']]['flow']) ? 'checked' : '';
			...
			foreach ($categorys as $k => $v) {
				...
				$str .= "<tr>
							...
							<td align='center'><input type='checkbox' name='priv[$modelid][]' $disabled $flow_check value='flow' ></td>
            		    </tr>";
			}	
