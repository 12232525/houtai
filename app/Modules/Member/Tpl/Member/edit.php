...
<div class="wrap J_check_wrap">
  ...
  <form name="myform" action="{:U('Member/edit')}" method="post" class="J_ajaxForm">
    ...
    <table width="100%" class="table_form">
		...
		<tr>
			<th>头像</th> 
			<td><img src="{:getavatar($data['userid'], 90, true)}" onerror="this.src='{$config_siteurl}statics/images/member/nophoto.gif'" height=90 width=90><input type="checkbox" name="delavatar" id="delavatar" class="input" value="1" ><label for="delavatar">删除头像</label></td>
		</tr>
		<tr>
			<th>是否审核</th> 
	...
	<?php foreach($forminfos['base'] as $k=>$v) {?>
		<tr>
			<th width="140"><?php echo $v['name']?></th> 
			<td><?php echo $v['form']?></td>
		</tr>
	<?php }?>
	<?php foreach($forminfos['senior'] as $k=>$v) {?>
		<tr>
			<th width="140"><?php echo $v['name']?></th> 
			<td><?php echo $v['form']?></td>
		</tr>
