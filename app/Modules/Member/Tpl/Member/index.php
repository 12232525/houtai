      ...
      <table width="100%" cellspacing="0">
        ...
        <tbody>
          <volist name="data" id="vo">
            <tr>
              <td align="center"><input type="checkbox" class="J_check" data-yid="J_check_y" data-xid="J_check_x"  value="{$vo.userid}" name="userid[]"></td>
              <td align="center"><if condition=" $vo['islock'] eq '1' "><img title="锁定" src="{$config_siteurl}statics/images/icon/icon_padlock.gif"></if>
              ...
              <td align="left"><img src="{:getavatar($vo['userid'],'90',true)}" height=18 width=18 onerror="this.src='{$config_siteurl}statics/images/member/nophoto.gif'">{$vo.username}<a href="javascript:member_infomation({$vo.userid}, '{$vo.modelid}', '')"><img src="{$config_siteurl}statics/images/icon/detail.png"></a></td>
              <td align="center">{$vo.nickname}</td>
              <td align="center">{$vo.email}</td>
              <td align="center">{$groupsModel[$vo['modelid']]}</td>
              <td align="center">{$vo.regip}</td>
              <td align="center"><if condition=" $vo['lastdate'] eq 0 ">还没有登录过
                  <else />
                  {$vo.lastdate|date='Y-m-d H:i:s',###}</if></td>
              <td align="center">{$vo.amount}</td>
              <td align="center">{$vo.point}</td>
              <td align="center">
                  <a href="javascript:member_infomation({$vo.userid}, '{$vo.modelid}', '')">&nbsp;查看&nbsp;</a>&nbsp;| 
              	  <a href="{:U('Member/edit', array('userid'=>$vo['userid']) )}">&nbsp;修改&nbsp;</a>&nbsp;|
              	  <a href="{:U('Member/edit', array('userid'=>$vo['userid']) )}">&nbsp;审核&nbsp;</a>  
              </td>
            </tr>
          </volist>
        </tbody>
