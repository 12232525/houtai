...
class PublicAction extends MemberbaseAction {
    ...
    //验证登陆
    public function public_doLogin() {
        ...
        if (empty($vCode) && $this->Member_config['openverification']) {
            $this->message(20031, array(), 'error');
        }
        if ($this->Member_config['openverification'] && !$this->verify($vCode, "userlogin")) {
            $this->message(20031, array(), 'error');
        }
        ...
        if ($userid > 0) {
            ...
            $this->message("登陆成功！", array(
                ...
                'uid' => $userid,
                'vip' => $userInfo['vip'],
                'avatar' => service("Passport")->user_avatar((int) $userid),
                'script' => $script,
            ));
        } else {
            //登陆失败
    ...
    //验证注册
    public function public_doRegister() {
        if (empty($this->Member_config['allowregister'])) {
        ...
        if ($info) {
            ...
            if ((int) $this->Member_config['choosemodel']) {
                if (!isset($info['modelid']) || empty($info['modelid'])) {
                    $info['modelid'] = (int) $this->Member_config['defaultmodelid'];
                } else {
                    //检查模型id是否合法
            ...
            if ($userid > 0) {
                ...
                //新注册用户积分
                $info['point'] = $this->Member_config['defualtpoint'] ? $this->Member_config['defualtpoint'] : 0;
                //新会员注册默认赠送资金
                $info['amount'] = $this->Member_config['defualtamount'] ? $this->Member_config['defualtamount'] : 0;
                //如果是邮箱验证，设置会员为未审核状态
                if (!empty($this->Member_config['enablemailcheck'])) {
                    $info['checked'] = 0;
                    $info['groupid'] = 7;
                } else {
                    //计算用户组
    ...
    public function public_checkEmail() {
        ...
        $status = service("Passport")->user_checkemail($email);
        if ($status) {
    ...
    public function public_checkUsername() {
        ...
        $status = service("Passport")->user_checkname($username);
        if ($status > 0) {
            $this->success('用户名可以使用！');
        } else {
            $this->error(service('Passport')->getError() ? service('Passport')->getError() : '用户名验证有误！');
        }
    }

    //检查昵称
    public function public_checkNickname() {
    ...
    public function connectregister() {
        ...
        switch ($connect_app) {
            case "qq":
                $this->qq_akey = $this->Member_config['qq_akey'];
                $this->qq_skey = $this->Member_config['qq_skey'];
                $connect['name'] = "QQ授权登陆";
                //取得授权用户基本信息
    ...
    public function doLostPassEmail() {
        ...
        $forgetpassword = $this->Member_config['forgetpassword'];
