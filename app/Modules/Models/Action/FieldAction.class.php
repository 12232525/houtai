...
class FieldAction extends AdminbaseAction {
	...
	public function listshow(){
		...
		if(IS_POST){
			...
			$icon = I('post.icon');
			$url = I('post.url');
			$tongji = I('post.tongji');
			$table = I('post.table');
			$column = I('post.column');
			 
			if(!$name || !$icon || !$url){
				$this->error('相关*号必填项数据错误！');
			}
			
			$setting = array();
			$count = count($sort);
			for($i = 0; $i < $count; $i++){
				if($name[$i] && $icon[$i] && $url[$i]){
					$setting[$sort[$i]]['sort'] = trim($sort[$i]);  
					$setting[$sort[$i]]['name'] = trim($name[$i]);
					$setting[$sort[$i]]['pos'] = trim($pos[$i]);
					$setting[$sort[$i]]['open'] = trim($open[$i]);
					$setting[$sort[$i]]['show'] = trim($show[$i]);
					$setting[$sort[$i]]['color'] = trim($color[$i]);
					$setting[$sort[$i]]['icon'] = trim($icon[$i]);
					$setting[$sort[$i]]['url'] = trim($url[$i]);
					$setting[$sort[$i]]['tongji'] = isset($tongji[$i]) ? intval($tongji[$i]) : '';
					$setting[$sort[$i]]['table'] = trim($table[$i]);
					$setting[$sort[$i]]['column'] = trim($column[$i]);
				}				
			}
			
			if(!$setting){
				$this->error('同一行记录相关*号必填项数据错误！');
			}
			
			ksort($setting);
			
			if(M('model')->where("modelid = $modelid")->save(array('showsetting'=>serialize($setting)))){
				$this->success("模型更新成功！");
			}else{
				$this->error('数据更新错误！');
			}
			...
			$data = M('model')->field('showsetting')->where("modelid = $modelid")->find();
			if($data){
				$setting = unserialize($data['showsetting']);
    ...
    public function edit() {
        ...
        if (IS_POST) {
			...
			//去掉选项类型下，多出的空行
			if(isset($post['setting']['options'])){
				array_pop($post['setting']['options']['sort']);
				array_pop($post['setting']['options']['name']);
				array_pop($post['setting']['options']['value']);
				array_pop($post['setting']['options']['color']);
			}
