...
<div class="wrap J_check_wrap">
	<div class="navpar">
	  <div class="nav">
	    <ul class="cc">
	      <li <if condition=" !isset($_GET['status']) ">class="current"</if>><a href="{:U('Contents/classlist', array('catid'=>$catid)  )}">{$catname}列表</a></li>
	      <li <if condition=" isset($_GET['status']) ">class="current"</if>><a href="{:U('Contents/classlist', array('catid'=>$catid ,'search'=>1 ,'status'=>1)  )}">待审核文章</a></li>
	    </ul>
	  </div>
  </div>
  <div class="mb10">
		<a href="javascript:void(0)" onClick="javascript:openwinx('{:U("Contents/add",array("catid"=>$catid))}','')" class="btn" title="添加内容"><span class="add"></span>添加内容</a>
  ...
  <form class="J_ajaxForm" action="" method="post">
      ...
      <table width="100%">
        ...
        <volist name="Content" id="vo">
          <tr>
            ...
            <td><a href="javascript:;;" onClick="javascript:openwinx('{:U("Contents/Contents/edit",array("catid"=>$vo['catid'],"id"=>$vo['id']))}','')">修改</a> | <a href="{:U("Contents/Contents/delete",array("catid"=>$vo['catid'],"id"=>$vo['id']))}" class="J_ajax_del" >删除</a> | <a  data-title="{$vo.title}" href="{:U('Comments/Comments/index',array('searchtype'=>2,'keyword'=>'c-'.$vo['catid'].'-'.$vo['id'].''))}" target="_blank">评论</a></td>
          </tr>
        </volist>
      </table>
    ...
    <div class="btn_wrap">
      <div class="btn_wrap_pd">
        ...
        <button class="btn J_ajax_submit_btn" type="submit" data-action="{:U('Contents/Contents/listorder',array('catid'=>$catid))}">排序</button>
        <button class="btn J_ajax_submit_btn" type="submit" data-action="{:U('Contents/Contents/public_check',array('catid'=>$catid))}">审核</button>
        <button class="btn J_ajax_submit_btn" type="submit" data-action="{:U('Contents/Contents/public_nocheck',array('catid'=>$catid))}">取消审核</button>
        <button class="btn J_ajax_submit_btn" type="submit" data-action="{:U('Contents/Contents/delete',array('catid'=>$catid))}">删除</button>
...
$(function () {
    Wind.use('ajaxForm','artDialog','iframeTools', function () {
        ...
        $('#J_Content_remove').click(function (e) {
            ...
            var $this = $(this);
            art.dialog.open("{$config_siteurl}index.php?g=Contents&m=Contents&a=remove&catid={$catid}&ids=" + id, {
                title: "批量移动"
            });
        });
    });
...
function pushs() {
    ...
    Wind.use('artDialog','iframeTools', function () {
         art.dialog.open("{$config_siteurl}index.php?g=Contents&m=Contents&a=push&action=position_list&modelid={$modelid}&catid={$catid}&id=" + id, {
            title: "信息推送"
        });
    });
}
