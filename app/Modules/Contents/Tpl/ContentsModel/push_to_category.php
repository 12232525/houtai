...
<div class="wrap_pop">
  <div class="nav">
    <ul class="cc">
      <li><a href="{:U('Contents/Contents/push',array('action'=>'position_list','modelid'=>$modelid,'catid'=>$catid,'id'=>$id))}">推送到推荐位</a></li>
      <li class="current"><a href="{:U('Contents/Contents/push',array('action'=>'push_to_category','modelid'=>$modelid,'catid'=>$catid,'id'=>$id))}">推送到其他栏目</a></li>
    </ul>
  </div>
  <form class="J_ajaxForm" action="{:U('Contents/push','action=push_to_category')}" method="post">
...
function change_siteid() {
    $("#load_catgory").load("{$config_siteurl}index.php?a=public_getsite_categorys&m=Contents&g=Contents&catid={$catid}");
}
//移除ID
