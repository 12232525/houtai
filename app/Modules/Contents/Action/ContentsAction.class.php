 ...
 */
class ContentsAction extends AdminbaseAction {
    ...
    private function getCategoryInfo(){
        ...
        //F('CategoryInfoModel', $modInfo);
        return $modInfo;
    }

    public function ajaxClassList(){
        $catInfo = $this->getCategoryInfo();
        //是否搜索
        ...
        if (!empty($catInfo)) {
            ...
            $data = $this->contentModel->field('id,title,views,updatetime')->where($where)->limit($page->firstRow . ',' . $page->listRows)->order(array("id" => "DESC"))->select();
        } else {
            $this->error("该栏目不存在！");
        }
        ...
        $res['count'] = $count;*/
        foreach ($data as &$val){
            $val['updatetime_format'] = date('Y-m-d', $val['updatetime']);
        }
        $res['list'] = $data;
    ...
    public function listorder() {
        ...
        if (is_array($listorders)) {
            ...
            $this->success("更新成功！", U("Contents/Contents/classlist", array('catid' => $this->catid)));
        } else {
            $this->error("参数错误！");
        }
    }
    ...
    public function public_categorys() {
        ...
        foreach ($categorys as $rs) {
            ...
            if ($rs['child'] == 0) {
                $data['target'] = "right";
                $data['url'] = U("Contents/Contents/classlist", array("catid" => $rs['catid']));
                //设置图标 
            ...
            if ($rs['type'] == 1 && $rs['child'] == 0) {
                $data['url'] = U("Contents/Contents/add", array("catid" => $rs['catid']));
                //设置图标 
