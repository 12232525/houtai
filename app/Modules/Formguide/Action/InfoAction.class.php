...
class InfoAction extends AdminbaseAction {
    ...
    public function public_view() {
        ...
        if (!$this->formid || !$dataid) {
            $this->error("该信息不存在！<script>setTimeout(function(){window.parent.art.dialog.list['check'].close();},1500);</script>");
        }
        if (empty($this->db)) {
            $this->error("该表单不存在！<script>setTimeout(function(){window.parent.art.dialog.list['check'].close();},1500);</script>");
        }
